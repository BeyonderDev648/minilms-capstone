...
router.post(
  ...
  [
    ...
    body('attachment_url').optional({ checkFalsy: true }).isURL().withMessage('Attachment must be a valid URL (starting with http:// or https://).'),
  ],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
    ...
    // Link-based attachment - no file storage needed. Could be a YouTube
    // link, an image URL, a Google Drive link, anything. The frontend
    // decides how to render it (embed/image/plain link) based on the URL shape.
    ...
    try {
      ...
      const result = await pool.query(
        'INSERT INTO lessons (course_id, title, content, attachment_url) VALUES ($1, $2, $3, $4) RETURNING *',
        [courseId, title, content, attachmentUrl]
      );
