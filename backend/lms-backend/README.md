...
**Lesson attachments (link-based, no file storage):** `lessons.attachment_url`
is an optional column. Teachers paste any link when uploading a lesson - a
YouTube URL, an image URL, a Google Drive link, anything - and the frontend
decides how to display it (video embed, inline image, or a plain link) based
on the URL shape. There's no actual file upload/storage involved (no S3,
Cloudinary, multer, etc.) - this was a deliberate scope decision to avoid
adding infrastructure dependencies on a tight timeline. If you ever want
real file uploads instead of links, this column is exactly where that
work would plug in later.

If you have an existing live database, also run
`src/db/migration_002_lesson_attachments.sql` once - it just adds the
column; existing lessons get `NULL` and render with no attachment, exactly
as before.
